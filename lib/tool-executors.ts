...
export const executeEditCode = async (
  ...
  try {
    ...
    // Safety check - this should never happen if error handling works correctly
    if (!matchInfo) {
      throw new Error(
        `Internal error: matchInfo is undefined after code matching in ${file_path}`
      );
    }

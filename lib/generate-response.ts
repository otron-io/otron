...
- Agent modes (triggered by labels or @ mention):
  - Work: add label "otron:work" (or legacy "codex-attempt"). Implement/fix, commit to a branch, and open/update a PR. If on an existing PR, resolve review comments and get it merge-ready.
  - Review: add label "otron:review" (or legacy "codex-review"/"codex-rust-review"). Perform granular, line-by-line review with code suggestions. Do not push commits in this mode.
  - Research: add label "otron:research". Research-only; produce a detailed answer or tech spec. No code changes.
  - Autonomous: mention "@otron-agent" anywhere (issue, PR body, or comment). Infer the correct mode and act. If ambiguous, ask one focused question before proceeding. Allowed to make code changes when appropriate.
- Status labels are updated automatically: <label>-in-progress while running, then <label>-completed.
- PR feedback: specific, constructive, testable. Reference files/lines and suggest concrete diffs where possible.
- Avoid double-triggering: do not add a label and mention @otron-agent at the exact same time.
- Be patient: the agent runs in CI and may take time to respond; avoid spamming.

Coding
- You do not push commits directly from this runtime. Use the GitHub coding agent (@otron-agent) by adding a mode label or mentioning it.
- Write actions: use "otron:work" (or legacy "codex-attempt"). Review-only: use "otron:review". Research-only: use "otron:research". For autonomous behavior, mention @otron-agent and it will infer the mode.
- The agent can traverse the codebase, make edits, and open PRs. Focus your requests on the outcome; the agent will handle the mechanics.
- To iterate on a PR: leave comments with requested changes and add the appropriate label (e.g., "otron:work" to apply changes, "otron:review" to review). Avoid tagging and labeling simultaneously to prevent duplicate runs.

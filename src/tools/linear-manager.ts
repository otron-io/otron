...
export class LinearManager {
   ...
   */
  async getIssueContext(issue: any, commentId?: string): Promise<string> {
   ...
   */
  async assignIssue(issueId: string, assigneeEmail: string): Promise<void> {
    try {
      // Fetch the issue
      const issue = await this.linearClient.issue(issueId);
